...
public class QuantityForPriceCampaignImplTest {
    ...
    public void apply() {
        ...
        Assert.assertEquals(new BigDecimal(2.7).round(new MathContext(2)), campaignPrice.round(new MathContext(2)));
    }
    ...
    public void apply_largeVal() {
        Product product = StoreServiceImpl.getInstance().getProduct("2");
        Campaign campaign = new QuantityForPriceCampaignImpl(product, 2 , BigDecimal.ONE);
        BigDecimal campaignPrice = campaign.apply(new CartItem(product.getProductId(), 150));
        Assert.assertEquals(new BigDecimal(75).round(new MathContext(2)), campaignPrice.round(new MathContext(2)));
    }
}